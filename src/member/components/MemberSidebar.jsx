...
export default function MemberSidebar() {
  ...
  const renderItem = ({ icon: Icon, label, path }) => {
    ...
    const handleClick = (e) => {
      if (path !== "/member-dashboard" && path !== "/member-sign-yourself" && path !== "/member-documents" && path !== "/member-contact-book" && path !== "/member-settings" && path !== "/member-templates") {
        e.preventDefault();
      }
    };
    ...
    return (
      <Link
        key={path}
        to={(path === "/member-dashboard" || path === "/member-sign-yourself" || path === "/member-documents" || path === "/member-contact-book" || path === "/member-settings" || path === "/member-templates") ? path : "#"}
        onClick={handleClick}
